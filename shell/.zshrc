...
__IAN[ITALIC_ON]=$'\e[3m'
__IAN[ITALIC_OFF]=$'\e[23m'
...
zstyle ':completion:*:descriptions' format %F{default}%B%{$__IAN[ITALIC_ON]%}--- %d ---%{$__IAN[ITALIC_OFF]%}%b%f
...
function -report-start-time() {
  ...
  if [ $ZSH_START_TIME ]; then
    ...
    export RPROMPT="%F{cyan}%{$__IAN[ITALIC_ON]%}${ELAPSED}%{$__IAN[ITALIC_OFF]%}%f $RPROMPT_BASE"
